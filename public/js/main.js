...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRnYmZxdG1mZmdrYW1mZ3Nra2tzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjI2MDkxNjYsImV4cCI6MjA3ODE4NTE2Nn0.qfePtu3UjQBNEy6bIYJnGSva4yZJ3H8PF5PK69MM_mQ"; // Get from Settings → API → anon/public key
...
const DEFAULT_PAYMENT_SETTINGS = {
  ...
  crypto: [
    ...
    { chain: "ETH", label: "Ethereum (ETH)", address: "TO_BE_SET_ETH_ADDRESS" },
    {
      chain: "USDT-ERC20",
      label: "USDT (ERC20)",
      address: "TO_BE_SET_USDT_ERC20",
    },
    {
      chain: "USDT-TRC20",
      label: "USDT (TRC20)",
      address: "TO_BE_SET_USDT_TRC20",
    },
  ],
};
...
async function loadPaymentSettings() {
  ...
  try {
    ...
    (data || []).forEach((row) => {
      if (row.method === "bank")
        grouped.bank = {
          label: row.label || "Bank Transfer",
          address: row.address,
        };
      else if (row.method === "paypal")
        grouped.paypal = { label: row.label || "PayPal", address: row.address };
      else if (row.method === "crypto")
        grouped.crypto.push({
          chain: row.chain || "CRYPTO",
          label: row.label || row.chain,
          address: row.address,
        });
    });
...
function initPaymentInstructions() {
  ...
  const updateCryptoOptionVisibility = (settings) => {
    ...
    const hasReal = (settings.crypto || []).some(
      (c) => c.address && !String(c.address).includes("TO_BE_SET")
    );
        ...
        btn.addEventListener("click", () => {
          ...
          if (box.childElementCount === 0 && window.QRCode) {
            ...
            const label =
              btn
                .closest(".card")
                .querySelector('div[style*="font-weight:600;"]').textContent ||
              "";
            let payload = address;
          ...
          box.style.display =
            box.style.display === "none" || box.style.display === ""
              ...
              : "none";
          btn.textContent =
            box.style.display === "block" ? "Hide QR" : "Show QR";
        });
      });
    } else {
      container.innerHTML = "";
...
async function handleEnrollment(event) {
  ...
  try {
    ...
    if (authError) {
      // If user already exists in Auth, show friendly error
      if (
        authError.message &&
    ...
    // 2. Insert user profile in users table (optional, for extra info)
    const { data, error } = await supabase
      .from("users")
      .insert([
        {
          email: email,
          // Do NOT store password_hash, Supabase Auth manages passwords
          ...
          auth_user_id: authData.user ? authData.user.id : null,
        },
      ])
      .select()
      .single();
    ...
    if (successMsg) {
      successMsg.textContent =
        "🎉 Enrollment successful! Please check your email to confirm your account.";
      successMsg.style.display = "block";
    }
