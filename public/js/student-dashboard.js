document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("userTier").textContent =
    user.tier?.toUpperCase() || "LEAD";

  // Load user's courses based on tier
  loadCourses(user.tier, supabaseClient);

  // Load payment submissions
  loadPaymentSubmissions(user.user_id, supabaseClient);

  // Load FTMO submissions
  loadFTMOSubmissions(user.user_id, supabaseClient);
});

async function loadCourses(tier, supabaseClient) {
...
async function loadPaymentSubmissions(userId, supabaseClient) {
...
async function loadFTMOSubmissions(userId, supabaseClient) {
  const { data, error } = await supabaseClient
